		...
		if($_POST['password'] != '' && strlen($_POST['password']) > 7)
		{
			if($_POST['password'] == $_POST['cmfpassword'])
			{
				if(!exist('users',' where username like "'.$_POST['username'].'" or email like "'.$_POST['email'].'"') && !exist('admins',' where username like "'.$_POST['username'].'" or email like "'.$_POST['email'].'"'))
				{
					...
					{
						unset($_POST['dealer']);
						$insert = insertAdmin(array('username'=>$_POST['username'],'email'=>$_POST['email'],'password'=>$_POST['password'],'name'=>$_POST['name'],'phone'=>$_POST['phone'],'city'=>$_POST['city'],'dealertype'=>$_POST['pdealertype'],'productstype'=>$_POST['productstype'],'company'=>$_POST['company'],'code'=>$_POST['code']));
						if($insert)
						{
							uploadimgs($insert);
