...
function getnoOfAdmins()
{
	...
	$result = mysql_query("select count(*) as count from admins where id <> 1");
...
function getAdmins($startResults,$resultsPerPage)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$result = mysql_query("select * from admins where id <> 1 LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = mysql_fetch_array($result); $i++)
		{
			...
			$allrows[$i]['productstype'] = $row['productstype'];
...
/*function updateAdmin($id,$username,$email,$company,$active)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$stmt = mysql_query("update admins set username = '$username',email = '$email',company = '$company',active = '$active' where id = '$id'");
	include("../libs/closedb.php");
}*/

function updateAdmin($id,$active)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$stmt = mysql_query("update admins set active = '$active' where id = '$id'");
	include("../libs/closedb.php");
}
...
function insertAdmin($username,$password,$email,$company,$active)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$stmt = mysql_query("insert into `admins` (`username`,`password`,`email`,`company`,`active`) values ('$username','$password','$email','$company','$active')");
	include("../libs/closedb.php");
}
