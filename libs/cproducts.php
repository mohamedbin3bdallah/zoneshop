...
function getProductByID($id,$admin)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$result = mysql_query("select * from products where id = '$id' and admin = '$admin'");
	$allrows = array();	
	if(!empty($result))
	{
		$row = mysql_fetch_array($result);
...
function getnoOfProducts($admin)
{
	include("../libs/config.php");
	include("../libs/opendb.php");	
	$result = mysql_query("select count(*) as count from products where admin = '$admin'");
	$row = mysql_fetch_array($result);
	include("../libs/closedb.php");
	return $row['count'];
}
...
function getProducts($admin,$startResults,$resultsPerPage)
{
	include("../libs/config.php");
	include("../libs/opendb.php");
	$result = mysql_query("select * from products where admin = '$admin' LIMIT $startResults, $resultsPerPage");
	$allrows = array();	
	if(!empty($result))
	{
		for($i=0; $row = mysql_fetch_array($result); $i++)
